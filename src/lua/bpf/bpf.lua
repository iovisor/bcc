...
local tracepoint_mt = {
	__index = {
		bpf = function (t, prog)
			...
			local prog_fd, err, log = S.bpf_prog_load2(S.c.BPF_PROG.TRACEPOINT, prog.insn, prog.pc)
...
local function trace_bpf(ptype, pname, pdef, retprobe, prog, pid, cpu, group_fd)
	...
	local prog_fd, err, log = S.bpf_prog_load2(S.c.BPF_PROG.KPROBE, prog.insn, prog.pc)
	assert(prog_fd, tostring(err)..': '..tostring(log))
	-- Open tracepoint and attach
...
return setmetatable({
	...
	socket = function (sock, prog)
		...
		local prog_fd, err, log = S.bpf_prog_load2(S.c.BPF_PROG.SOCKET_FILTER, prog.insn, prog.pc)
		assert(prog_fd, tostring(err)..': '..tostring(log))
