    ...
    def decode(cls, i, w, w1):
        try:
            ...
            if opclass == "dstimm":
                ...
                return "goto %s <%d>" % ("%+d" % (w.offset), i + w.offset + 1), 0
                ...
                return (
                    "if r%d %s %d goto pc%s <%d>"
                    % (w.dst, op, w.imm, "%+d" % (w.offset), i + w.offset + 1),
                    0,
                )

            elif opclass == "jdstsrcoff":
                return (
                    "if r%d %s r%d goto pc%s <%d>"
                    % (w.dst, op, w.src, "%+d" % (w.offset), i + w.offset + 1),
                    0,
                )
                ...
                return "r0 = *(u%d*)skb[r%d %s]" % (bits, w.src, "%+d" % (w.imm)), 0

            elif opclass == "ldstsrcoff":
                return (
                    "r%d = *(u%d*)(r%d %s)" % (w.dst, bits, w.src, "%+d" % (w.offset)),
                    0,
                )

            elif opclass == "sdstoffimm":
                return (
                    "*(u%d*)(r%d %s) = %d" % (bits, w.dst, "%+d" % (w.offset), w.imm),
                    0,
                )

            elif opclass == "sdstoffsrc":
                return (
                    "*(u%d*)(r%d %s) = r%d" % (bits, w.dst, "%+d" % (w.offset), w.src),
                    0,
                )
                ...
                if w.src != cls.BPF_PSEUDO_CALL:
                    try:
                        return (
                            "%s bpf_%s#%d" % (name, cls.bpf_helpers[w.immu], w.immu),
                            0,
                        )
                    except IndexError:
...
class MapDecoder:
    ...
    def print_ct_map(cls, t, indent="", offset=0, sizeinfo=False):
        ...
        try:
            for field_name, field_type in t._fields_:
                is_structured = issubclass(field_type, ct.Structure) or issubclass(
                    field_type, ct.Union
                )
                field_type_name = cls.get_ct_name(field_type)
                field_offset = getattr(t, field_name).offset
                field_size = ct.sizeof(field_type)
                sizedesc = cls.format_size_info(
                    offset + field_offset, field_size, sizeinfo
                )
                if is_structured:
                    map_lines.append("%s%s%s {" % (indent, sizedesc, field_type_name))
                    map_lines += cls.print_ct_map(
                        field_type, indent + "  ", offset + field_offset
                    )
                    map_lines.append("%s} %s;" % (indent, field_name))
                else:
                    map_lines.append(
                        "%s%s%s %s;" % (indent, sizedesc, field_type_name, field_name)
                    )
        except ValueError:
            # is a bit field
            offset_bits = 0
            for field in t._fields_:
                if len(field) == 3:
                    ...
                    field_type_name = cls.get_ct_name(field_type)
                    sizedesc = cls.format_size_info(
                        offset, offset_bits, sizeinfo, field_bits
                    )
                    map_lines.append(
                        "%s%s%s %s:%d;"
                        % (indent, sizedesc, field_type_name, field_name, field_bits)
                    )
                else:
    ...
    def print_map_ctype(cls, t, field_name, sizeinfo):
        is_structured = issubclass(t, ct.Structure) or issubclass(t, ct.Union)
