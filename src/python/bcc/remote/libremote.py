...
class LibRemote(object):
    def __init__(self, remote_name, remote_arg=None):
        ...
        cls = globals()[remote_name.capitalize() + 'Remote']
    ...
    def _remote_send_command(self, cmd):
        ...
        if ret[0].startswith(b'Command not recognized'):
            print('Command not recognized! cmd: {}'.format(cmd))
            return (-1, [])

        if len(ret) > 1 and ret[1].startswith(b'free(): '):
            print('BPFd crashed ({}) while processing cmd: {}'.format(ret[1], cmd))
            print('To debug, run with BCC_REMOTE_DEBUG=1 env var and look at the BPFd core dumps')
            os._exit(1) # exit immediately, we can't do cleanups as bpfd is dead

        if ret[0].startswith(b'Open failed, ignoring'):
            return (-1, [])

        # Assume success if first list element doesn't have ret=
        if not b'ret=' in ret[0]:
            return (0, ret)

        m = re.search(b"ret=(\-?\d+)", ret[0])
        if m == None:
    ...
    def bpf_attach_kprobe(self, fd, t, evname, fnname, offset, maxactive=0):
        evname = evname.decode("utf-8")
        fnname = fnname.decode("utf-8")
        cmd = "BPF_ATTACH_KPROBE {} {} {} {} {} {}".format(
            fd, t, evname, fnname, offset, maxactive)
        ret = self._remote_send_command(cmd)
        return ret[0]
    ...
    def bpf_prog_load(self, prog_type, name, func_str, license_str, kern_version):
        ...
        license_str = license_str.decode("utf-8")
        func_len = len(func_str)
        func_str = base64.b64encode(func_str).decode("utf-8")
        cmd = "BPF_PROG_LOAD {} {} {} {} {} {}".format(
            prog_type, name, func_len, license_str, kern_version, func_str)
        ret = self._remote_send_command(cmd)
        return ret[0]

    def bpf_create_map(self, data, allow_rlimit):
        attr = ct.cast(data, ct.POINTER(BpfCreateMapAttr)).contents
        name = attr.name.decode("utf-8")
        cmd = "BPF_CREATE_MAP {} {} {} {} {} {} {}".format(
            attr.map_type, name, attr.key_size, attr.value_size, attr.max_entries,
            attr.map_flags, int(allow_rlimit == True))
        ret = self._remote_send_command(cmd)
    ...
    def bpf_update_elem(self, map_fd, kstr, klen, lstr, llen, flags):
        kstr = kstr.decode("utf-8")
        lstr = lstr.decode("utf-8")
        cmd = "BPF_UPDATE_ELEM {} {} {} {} {} {}".format(
            map_fd, kstr, klen, lstr, llen, flags)
        ret = self._remote_send_command(cmd)
        return ret[0]
    ...
    def bpf_lookup_elem(self, map_fd, kstr, klen, llen):
        ...
        cmd = "BPF_LOOKUP_ELEM {} {} {} {}".format(map_fd, kstr.decode("utf-8"), klen, llen)
        ret = self._remote_send_command(cmd)
        return ret
    ...
    def ksym(self, addr):
        ...
        if ret_code < 0:
            ...
            name, offset, module = ret[1][1].split(b";")
