...
static __u64 ptr_to_u64(void *ptr) { return (__u64)(unsigned long)ptr; }

extern "C" int bpf_create_map(enum bpf_map_type map_type, int key_size,
...
extern "C" int bpf_update_elem(int fd, void *key, void *value,
...
extern "C" int bpf_lookup_elem(int fd, void *key, void *value) {
  union bpf_attr attr;
  memset(&attr, 0, sizeof(attr));
  attr.map_fd = fd;
  attr.key = ptr_to_u64(key);
...
extern "C" int bpf_delete_elem(int fd, void *key) {
  union bpf_attr attr;
  memset(&attr, 0, sizeof(attr));
  attr.map_fd = fd;
  attr.key = ptr_to_u64(key);
...
extern "C" int bpf_get_first_key(int fd, void *key, size_t key_size) {
  union bpf_attr attr;
  int i, res;
...
extern "C" int bpf_get_next_key(int fd, void *key, void *next_key) {
  union bpf_attr attr;
  memset(&attr, 0, sizeof(attr));
  attr.map_fd = fd;
  attr.key = ptr_to_u64(key);
...
extern "C" int bpf_prog_load(enum bpf_prog_type prog_type,
...
extern "C" int bpf_open_raw_sock(const char *name) {
...
extern "C" int bpf_attach_socket(int sock, int prog) {
...
extern "C" void *bpf_attach_kprobe(int progfd,
  ...
  reader =
      perf_reader_new(cb, NULL, NULL, cb_cookie, probe_perf_reader_page_cnt);
  if (!reader)
    goto error;
...
extern "C" void *bpf_attach_uprobe(int progfd,
                                   enum bpf_probe_attach_type attach_type,
                                   const char *ev_name, const char *binary_path,
                                   uint64_t offset, pid_t pid, int cpu,
                                   int group_fd, perf_reader_cb cb,
                                   void *cb_cookie) {
  int kfd;
  char buf[PATH_MAX];
  char new_name[128];
...
extern "C" int bpf_detach_kprobe(const char *ev_name) {
  char buf[256];
...
extern "C" int bpf_detach_uprobe(const char *ev_name) {
  return bpf_detach_probe(ev_name, "uprobe");
}

extern "C" void *bpf_attach_tracepoint(int progfd, const char *tp_category,
...
extern "C" int bpf_detach_tracepoint(const char *tp_category,
                                     const char *tp_name) {
...
extern "C" void *bpf_open_perf_buffer(perf_reader_raw_cb raw_cb,
                                      perf_reader_lost_cb lost_cb,
                                      void *cb_cookie, int pid, int cpu,
                                      int page_cnt) {
  int pfd;
  struct perf_event_attr attr = {};
  struct perf_reader *reader = NULL;
  ...
  attr.wakeup_events = 1;
  pfd =
      syscall(__NR_perf_event_open, &attr, pid, cpu, -1, PERF_FLAG_FD_CLOEXEC);
  if (pfd < 0) {
    fprintf(stderr, "perf_event_open: %s\n", strerror(errno));
    fprintf(stderr,
            "   (check your kernel for PERF_COUNT_SW_BPF_OUTPUT support, 4.4 "
            "or newer)\n");
    goto error;
  }
  perf_reader_set_fd(reader, pfd);
...
extern "C" int bpf_open_perf_event(uint32_t type, uint64_t config, int pid,
                                   int cpu) {
  int fd;
  struct perf_event_attr attr = {};
...
extern "C" int bpf_attach_perf_event(int progfd, uint32_t ev_type,
                                     uint32_t ev_config, uint64_t sample_period,
                                     uint64_t sample_freq, pid_t pid, int cpu,
                                     int group_fd) {
  ...
  int fd = syscall(__NR_perf_event_open, &attr, pid, cpu, group_fd,
                   PERF_FLAG_FD_CLOEXEC);
  if (fd < 0) {
...
extern "C" int bpf_close_perf_event_fd(int fd) {
  int res, error = 0;
  if (fd >= 0) {
