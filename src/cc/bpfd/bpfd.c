...
int bpf_prog_load_handle(int type, char *name, char *bin_b64, int prog_len,
  ...
  ret = bcc_prog_load((enum bpf_prog_type)type, name, insns, prog_len,
                      (const char *)license, kern_version, 0, NULL, 0);

  printf("bcc_prog_load: ret=%d\n", ret);
  free(bin_buf);
  return ret;
}
...
int bpf_clear_map(int map_fd, int klen) {
  ...
  int count = 0, ret = -ENOMEM, same_keys = 0;
...
int main(int argc, char **argv) {
  ...
  while (fgets(line_buf, LINEBUF_SIZE, stdin)) {
    ...
    if (!strcmp(in->cmd, "GET_KALLSYMS")) {
      ...
      int ret, prog_fd, type, maxactive;
      ...
      ret = bpf_attach_kprobe(prog_fd, type, ev_name, fn_name, offset, maxactive);
      ...
      int ret, map_type, allow_rlimit;
      char *name;

      PARSE_INT(map_type);
      PARSE_STR(name);
      PARSE_UINT32(attr.key_size);
      PARSE_UINT32(attr.value_size);
      PARSE_UINT32(attr.max_entries);
      PARSE_UINT32(attr.map_flags);
      PARSE_INT(allow_rlimit);
      attr.map_type = map_type;
      attr.name = name;

      ret = bcc_create_map_xattr(&attr, allow_rlimit);
      printf("bcc_create_map: ret=%d\n", ret);
