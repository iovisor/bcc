...
int count_num_tokens(const char *str) {
  ...
  for (token = strtok(str_copy, " "); token != NULL; token = strtok(NULL, " "))
    num_tokens++;
...
struct user_input *parse_user_input(const char *str) {
  ...
  in = (struct user_input *) calloc(1, sizeof(struct user_input));

  if (num_tokens > 1) {
    in->num_args = num_tokens - 1;
    in->args = (char **) calloc(in->num_args, sizeof(char *));
  }
  ...
  for (i = 0; i < in->num_args; i++) {
    ...
    if (!token)
      continue;
    in->args[i] = (char *)malloc(strlen(token) + 1);
    strcpy(in->args[i], token);
  }
