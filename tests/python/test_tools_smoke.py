...
class SmokeTests(TestCase):
    ...
    def run_with_duration(self, command, timeout=10):
        ...
        self.assertEqual(
            0,  # clean exit
            subprocess.call(
                "timeout -s KILL %ds %s > /dev/null" % (timeout, full_command),
                shell=True,
            ),
        )
    ...
    def run_with_int(
        self, command, timeout=5, kill_timeout=5, allow_early=False, kill=False
    ):
        full_command = TOOLS_DIR + command
        signal = "KILL" if kill else "INT"
        rc = subprocess.call(
            "timeout -s %s -k %ds %ds %s > /dev/null"
            % (signal, kill_timeout, timeout, full_command),
            shell=True,
        )
        ...
        self.assertTrue(
            (rc == 0 and allow_early) or rc == 124 or (rc == 137 and kill),
            "rc was %d" % rc,
        )
