...
"    runqslower -c bash # trace bash only (max comm name length is 15 chars)\n"
...
static error_t parse_arg(int key, char *arg, struct argp_state *state)
{
	...
	case 'c':
		env.comm = strdup(arg);
		len = strlen(arg) + 1;
		env.task_comm_len = MIN(len,TASK_COMM_LEN);
		if (len > TASK_COMM_LEN) {
			fprintf(stderr, "Warning: Command name '%.*s...' (%zu chars) exceeds max %d chars. "
			"Only first %d characters will be used.\n",
			 TASK_COMM_LEN, arg, strlen(arg), TASK_COMM_LEN - 1, TASK_COMM_LEN - 1);
			argp_usage(state);
		}
		break;
	case 'P':
...
int main(int argc, char **argv)
{
	...
	if (env.comm)
                strncpy((char*)obj->rodata->targ_comm, env.comm, env.task_comm_len);
