...
static const struct argp_option opts[] = {
	...
	{ "pid", 'p', "PID", 0, "Detect memory leak on the specified process" },
	{ "stop-the-world", 'w', NULL, 0, "Stop the target process during tracing" },
	{ "command", 'c', "COMMAND", 0, "Execute and detect memory leak on the specified command" },
	{ "interval", 'i', "INTERVAL", 0, "Set interval in second to detect leak" },
	{ "top", 'T', "TOP", 0, "Report only specified amount of backtraces"},
	{ "suppressions", 's', "SUPPRESSIONS", 0, "Suppressions file name"},
	{},
};
...
struct lsan_info_ext_t *allocs = NULL;
...
{
	struct lsan_info_ext_t *curr = NULL;
	struct lsan_info_ext_t *next = NULL;
...
{
	struct lsan_info_ext_t *val = NULL;
	__u64 key = 0;
	__u64 mid = 0;
...
{
	int alignment = 1; /* Alignment in bytes, 1 means no alignment */
	__u64 pp = begin;
	__u64 p = 0;
	__u64 chunk = 0;
	struct lsan_info_ext_t *val = NULL;
...
static int read_table(void)
{
	...
	struct lsan_info_ext_t *item = NULL;
	...
	while (!bpf_map_get_next_key(afd, &lookup_key, &address)) {
		...
		item = (struct lsan_info_ext_t*)malloc(sizeof(struct lsan_info_ext_t));
		ON_MEM_FAILURE(item);
...
static void delete_hash_lsan_hash(struct lsan_info_ext_t *hash) {
	struct lsan_info_ext_t *curr = NULL;
	struct lsan_info_ext_t *next = NULL;
