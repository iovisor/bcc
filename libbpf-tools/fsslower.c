 ...
 *
 * Based on fsdist and previous work by
 *     - Brendan Gregg
 *     - Dina Goldshtein
 *     - Joe Yin
 *     - Samuel Nair
 *     - Wenbo Zhang
 * 27-May-2021   Hengqi Chen   Created this.
 */
#include <argp.h>
#include <libgen.h>
#include <signal.h>
...
static void handle_event(void *ctx, int cpu, void *data, __u32 data_sz)
{
	...
	if (csv) {
		...
		e->size == LLONG_MAX ? printf("LL_MAX,") : printf("%ld,", e->size);
	...
	e->size == LLONG_MAX ? printf("%-7s ", "LL_MAX") : printf("%-7ld ", e->size);
...
int main(int argc, char **argv)
{
	...
	skel->rodata->min_lat_ms = min_lat_ms;
