...
static int handle_event(void *ctx, void *data, size_t len)
{
	...
	if (display_cgroup) {
		cgroup_cgroup_path(e->fcgroupid, cgroup_path, sizeof(cgroup_path));
		printf(" CGROUP %lld (\"%s\"),", e->fcgroupid, cgroup_path);
		if (e->tcgroupid) {
			if (e->tcgroupid != e->fcgroupid)
				cgroup_cgroup_path(e->tcgroupid, cgroup_path,
						   sizeof(cgroup_path));
			printf(" MEMCG %lld (\"%s\"),", e->tcgroupid, cgroup_path);
		}
	}
