...
int BPF_PROG(fentry__vfs_create, struct inode *dir, struct dentry *dentry)
{
	return probe_create(dir, dentry);
}

SEC("fentry/security_inode_create")
int BPF_PROG(fentry__security_inode_create, struct inode *dir,
...
int BPF_PROG(fentry__vfs_unlink, struct inode *dir, struct dentry *dentry)
{
	u64 id = bpf_get_current_pid_tgid();
	struct event event = {};
