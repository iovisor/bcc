...
void handle_event(void *ctx, int cpu, void *data, __u32 data_sz)
{
	...
	if (env.csv)
		printf("%lld,%s,%d,%c,%ld,%lld,%lld,%s\n", e->end_ns, e->task, e->tgid,
			e->type, e->size, e->offset, e->delta_us, e->file);
	else
		printf("%-8s %-14.14s %-6d %c %-7ld %-8lld %7.2f %s", ts, e->task,
			e->tgid, e->type, e->size, e->offset / 1024,
