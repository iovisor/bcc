...
static int handle_switch(u64 *ctx, struct task_struct *prev, struct task_struct *next)
{
	...
	if (allow_record(prev)) {
		...
		bpf_probe_read_kernel_str(&val.comm, bpf_core_field_size(prev->comm), prev->comm);
		val.delta = 0;
