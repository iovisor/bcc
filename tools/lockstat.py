...
# 26-Mar-2018   Gisle Dankel   Refactored
#  5-Apr-2018   Gisle Dankel   Added filter and display options
#
#
...
{0} -C blocked.sum,tid.count,lock file
        Show these columns only.
{0} -g comm -s blocked.max file
...
parser.add_argument("-C", "--columns", type=column_name_list,
                    ...
                    help="Include these columns. " +
                         ...
                         default_columns)
parser.add_argument("-c", "--callstacks", action="store_true",
                    help="Display full callstacks")
parser.add_argument("tracefile", type=str,
                    help="tracefile generated by locktrace")
...
def print_summary_for_pid(pid, stats, duration):
    ...
    summary = textwrap.dedent("""
        ...
        Tracing overhead:                  %ims (%i%%/run)
        Pre-empted:                        %s (%s/run + preempt)
    """ % (
        len(set(stats['tid'])),
        len(set(stats['addr'])),
        len(set(stats['comm'])),
...
formatters = {
    ...
    'blocked_us': Formatter('Blocked', 'Sum ms', 7, us_val_fmt),
    'sys_futex_us': Formatter('SysFutx', 'Sum ms', 7, us_val_fmt),
    ...
    'usr_syms': Formatter('Caller', '', 40, lambda v: caller_string(stack_syms(v))),
...
def add_derived_stats(total, sorted_stats, group_by, order_by):
    total['avg_blocked_us'] = round(total['blocked_us'] / total['blocked_count'])
    total['sys_futex_count'] = total['wait_count'] + total['wake_count']
    total['avg_sys_futex_us'] = round(total['sys_futex_us'] / total['sys_futex_count'])
    for dim in stat_dims:
        total[dim + "_count"] = len(total[dim])
        if dim == group_by:
            total[dim] = sorted_stats[dim][0]
        else:
            total[dim] = sorted_stats.sum(dim).sort(order_by)[dim][0]


def print_stats_for_pid(pid, stats, full_stacks, order_by, group_by, columns):
    indent = "  "
    ...
    if group_by != 'usr_syms':
        columns.append(group_by)
    columns.append('usr_syms')

    # Header
    widths = [formatters[col].width for col in columns]
    ...
    stats.set_aggregators({'max_blocked_us': max, 'max_sys_futex_us': max})
    ...
    for sub_stats in sorted(
        ...
        print(indent + row)
        if full_stacks:
            stack_indent = " " * (len(header) - formatters['usr_syms'].width)
            for sym in stack_syms(subtotal['usr_syms']):
...
(meta, stats) = read_tracefile(args.tracefile, force_types)
for pid, pid_stats in stats.items():
    if args.pid and pid is not args.pid:
        continue
    events = pid_stats
    if args.event == 'lock':
    ...
    total_blocked = us_to_s(events.sum_all()['blocked_us'])
    if (total_blocked / meta['duration']) == 0:
        comm = events.sort('blocked_us')[0]['comm']
        print("Skipping %s: %i" % (comm, total_blocked))
        continue

    print("Pid %d: (%d records)" % (pid, pid_stats.count()))
    print_summary_for_pid(pid, pid_stats, meta['duration'])
    print_stats_for_pid(
        pid, events, args.callstacks, args.sort_by, args.group_by,
        args.columns)
