...
if not folded:
    print(
        "Tracing off-CPU time (us) of %s by %s stack" % (thread_context, stack_context),
        end="",
    )
...
for k, v in sorted(counts.items(), key=lambda counts: counts[1].value):
    ...
    kernel_stack = [] if k.kernel_stack_id < 0 else stack_traces.walk(k.kernel_stack_id)
    ...
    if folded:
        ...
        if not args.kernel_stacks_only:
            if stack_id_err(k.user_stack_id):
                ...
                line.extend(
                    [
                        b.sym(addr, k.tgid).decode("utf-8", "replace")
