...
def _getParser():
    ...
    a(
        ...
        "--timestamp",
        action="store_true",
        help="include timestamp (local time default)",
    )
...
def _embedded_c(args):
    ...
    code_substitutions = [
        ...
        (
            "FILTER_PID",
            "0" if not Global.args.pid else "task->tgid != %s" % Global.args.pid,
        ),
        ...
        (
            ...
            else "(task->start_time.tv_sec * 1000000000L) + task->start_time.tv_nsec",
        ),
    ]
...
def _ebpf_comment():
    ...
    args = (
        str(vars(Global.args))
        .replace("{", "{\n\t")
        .replace(", ", ",\n\t")
        .replace("}", ",\n }\n\n")
    )
    return "\n   /*" + (
        "\n %s\n\n ARGV = %s\n\n ARGS = %s/" % (comment, " ".join(Global.argv), args)
    ).replace("\n", "\n\t*").replace("\t", "    ")
...
def _print_event(cpu, data, size):  # callback
    ...
    print(
        "%-16s %-6d %-6d %-6d %-7.2f " % (e.task.decode(), e.pid, e.ppid, e.tid, age),
        end="",
    )
    if e.sig_info == 0:
...
def initialize(arg_list=sys.argv[1:]):
    ...
    for _ in range(2):
        ...
        try:
            ...
            error = format(e)
            if (
                not Global.args.timespec
                and error.find("struct timespec")
                and error.find("start_time")
            ):
                print(
                    "This kernel keeps task->start_time in a struct timespec.\n"
