...
"""
        ...
        text = text.replace(
            "SIGNATURE", "" if len(self.signature) == 0 else ", " + self.signature
        )
...
%s.update(&__pid, &__time);
    ...
    def _generate_entry_probe(self):
        ...
        for arg in re.finditer(regex, self.filter):
            self.args_to_probe.add(arg.group(1))
        if (
            any(map(lambda expr: "$latency" in expr, self.exprs))
            or "$latency" in self.filter
        ):
    ...
    def _attach_entry_probe(self):
        if self.is_user:
            ...
            self.bpf.attach_kprobe(event=self.function, fn_name=self.entry_probe_func)

    def _bail(self, error):
        raise ValueError("error parsing probe '%s': %s" % (self.raw_spec, error))
    ...
    def _validate_specifier(self):
        ...
        if len(parts) < 3:
            self._bail(
                "at least the probe type, library, and "
        ...
        if parts[0] not in ["r", "p", "t", "u"]:
            self._bail(
                "probe type must be 'p', 'r', 't', or 'u'" + " but got '%s'" % parts[0]
            )
        if re.match(r"\S+\(.*\)", parts[2]) is None:
            self._bail(
                ("function signature '%s' has an invalid " + "format") % parts[2]
            )
    ...
    def __init__(self, tool, type, specifier):
        ...
        self.label = spec_and_label[1] if len(spec_and_label) == 2 else None
        ...
        if not self.is_default_expr:
            ...
            self.expr_types = ["u64" if not self.probe_type == "r" else "int"]
            self.exprs = ["1" if not self.probe_type == "r" else "$retval"]
...
static inline bool %s(char const *ignored, char const *str) {
    ...
    def _generate_usdt_arg_assignment(self, i):
        ...
        if self.probe_type == "u" and expr[0:3] == "arg":
            ...
            arg_ctype = self.usdt_ctx.get_probe_arg_ctype(self.function, arg_index - 1)
            return (
    ...
    def _generate_hash_decl(self):
        if self.type == "hist":
            return "BPF_HISTOGRAM(%s, %s);" % (self.probe_hash_name, self.expr_types[0])
        else:
    ...
    def _generate_hash_update(self):
        if self.type == "hist":
            return "%s.increment(bpf_log2l(__key));" % self.probe_hash_name
        else:
...
"""
    ...
    def _attach_k(self):
        if self.probe_type == "t":
            ...
            self.bpf.attach_kretprobe(event=self.function, fn_name=self.probe_func_name)
        else:
            self.bpf.attach_kprobe(event=self.function, fn_name=self.probe_func_name)
    ...
    def _display_key(self, key):
        if self.is_default_expr:
            ...
            def str_i(i):
                ...
                return "%s = %s" % (self._display_expr(i), key_i)
            ...
            for key, value in sdata:
                ...
                if self.is_default_expr:
                    if not self.probe_type == "r":
                        ...
                        key_str = "retval = %s" % self._v2s(key.v0)
                else:
                    key_str = self._display_key(key)
                print("\t%-10s %s" % (str(value.value), key_str))
        elif self.type == "hist":
            label = self.label or (
                self._display_expr(0) if not self.is_default_expr else "retval"
            )
...
"""
    ...
    def __init__(self):
        ...
        parser.add_argument(
            "-p", "--pid", type=int, help="id of the process to trace (optional)"
        )
        parser.add_argument(
            "-z",
            "--string-size",
        ...
        parser.add_argument(
            "-n", "--number", type=int, dest="count", help="number of outputs"
        )
        parser.add_argument(
            "-v",
            "--verbose",
...
#include <uapi/linux/ptrace.h>
        ...
        bpf_source += BPF.generate_auto_includes(map(lambda p: p.raw_spec, self.probes))
        for probe in self.probes:
            bpf_source += probe.generate_text()
        if self.args.verbose:
            for text in [
                probe.usdt_ctx.get_text() for probe in self.probes if probe.usdt_ctx
            ]:
                print(text)
            print(bpf_source)
        usdt_contexts = [probe.usdt_ctx for probe in self.probes if probe.usdt_ctx]
    ...
    def _main_loop(self):
        ...
        while True:
            ...
            count_so_far += 1
            if self.args.count is not None and count_so_far >= self.args.count:
                exit()
