    ...
    def attach(self):
        if self.type == "p":
            if self.library:
                ...
                self.bpf.attach_kprobe(event_re=self.pattern, fn_name="trace_count")
                self.matched = self.bpf.num_open_kprobes()
        elif self.type == "t":
            self.bpf.attach_tracepoint(tp_re=self.pattern, fn_name="trace_count")
...
BPF_STACK_TRACE(stack_traces, 1024);
        ...
        if self.per_pid:
            ...
            trace_count_text = trace_count_text.replace(
                "STORE_COMM", "bpf_get_current_comm(&key.name, sizeof(key.name));"
            )
        else:
...
class Tool(object):
    ...
    ./stackcount -U t:sched:sched_switch   # user stacks only
        ...
        parser.add_argument(
            "-s", "--offset", action="store_true", help="show address offsets"
        )
        parser.add_argument(
            "-P",
            "--perpid",
        ...
        parser.add_argument(
            ...
            "--user-stacks-only",
            action="store_true",
            help="user stack only",
            default=False,
        )
        parser.add_argument(
            "-v", "--verbose", action="store_true", help="show raw addresses"
        )
        parser.add_argument(
            "-d",
            "--delimited",
        ...
        parser.add_argument(
            "-f", "--folded", action="store_true", help="output folded format"
        )
        parser.add_argument(
            "--debug",
            action="store_true",
        ...
        parser.add_argument("pattern", help="search expression for events")
        ...
        if self.args.kernel_stacks_only and self.args.user_stacks_only:
            print(
                "ERROR: -K and -U are mutually exclusive. If you want "
    ...
    def run(self):
        ...
        while True:
            ...
            for k, v in sorted(counts.items(), key=lambda counts: counts[1].value):
                user_stack = (
                    [] if k.user_stack_id < 0 else stack_traces.walk(k.user_stack_id)
                )
                kernel_stack = (
                    []
                    if k.kernel_stack_id < 0
                    else stack_traces.walk(k.kernel_stack_id)
                )
