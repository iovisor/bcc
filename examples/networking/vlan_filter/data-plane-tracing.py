...
def help():
    ...
    print(
        "   -k kafka_server_name     select kafka server name. Default is save to file"
    )
    print(
        "                            If -k option is not specified data will be saved to file."
    )

    print("")
    print("examples:")
    print(
        "    data-plane-tracing                                      # bind socket to eth0"
    )
    print(
        "    data-plane-tracing -i eno2 -k vc.manage.overcloud:9092  # bind socket to eno2 and send data to kafka server in iovisor-topic."
    )
    exit()
...
while 1:
    ...
    src_host_ip = (
        str(packet_bytearray[26])
        + "."
        + str(packet_bytearray[27])
        + "."
        + str(packet_bytearray[28])
        + "."
        + str(packet_bytearray[29])
    )
    dest_host_ip = (
        str(packet_bytearray[30])
        + "."
        + str(packet_bytearray[31])
        + "."
        + str(packet_bytearray[32])
        + "."
        + str(packet_bytearray[33])
    )
    ...
    src_vm_ip = (
        str(packet_bytearray[80])
        + "."
        + str(packet_bytearray[81])
        + "."
        + str(packet_bytearray[82])
        + "."
        + str(packet_bytearray[83])
    )
    dest_vm_ip = (
        str(packet_bytearray[84])
        + "."
        + str(packet_bytearray[85])
        + "."
        + str(packet_bytearray[86])
        + "."
        + str(packet_bytearray[87])
    )
