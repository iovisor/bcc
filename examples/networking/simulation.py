...
class Simulation(object):
    ...
    def _ns_add_ifc(
        ...
        if name in self.ipdbs:
            ...
            if disable_ipv6:
                cmd1 = ["sysctl", "-q", "-w", "net.ipv6.conf.default.disable_ipv6=1"]
        ...
        if in_ifc:
            ...
            try:
                out_ifc = self.ipdb.create(
                    ifname="%sa" % ifc_base_name,
                    kind="veth",
                    peer="%sb" % ifc_base_name,
                ).commit()
    ...
    def _create_ns(
        ...
        (ns_ipdb, out_ifc, in_ifc) = self._ns_add_ifc(
            name,
            "eth0",
            name,
            in_ifc,
    ...
    def release(self):
        ...
        for p in self.processes:
            ...
            try:
                ...
                p.release()
        for name, db in self.ipdbs.items():
            db.release()
