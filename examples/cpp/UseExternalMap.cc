 ...
 * created map by calling libbpf bpf_create_map2.
 *
...
int main() {
  ...
  // create a map through bpf_create_map2, bcc knows nothing about this map.
  ctrl_map_fd = bpf_create_map2(BPF_MAP_TYPE_ARRAY, "control", sizeof(uint32_t),
                                sizeof(uint32_t), 1, 0);
  CHECK(ctrl_map_fd < 0, "bpf_create_map2 failure");
