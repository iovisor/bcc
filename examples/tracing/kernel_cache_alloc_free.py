...
#		
#
# Written as a basic tool example of using ePBF to track of kernel cache allocation and deallocation process.
#
#
...
/* Remember : When using tracepoints we need to make use of /sys/kernel/debug/tracing/events directory.

For example : the parameters of the function kmem_cache_alloc are defined in /sys/kernel/debug/tracing/events/kmem/kmem_cache_alloc/format
 */
...
class Data(ct.Structure):
    _fields_ = [("timestamp", ct.c_ulonglong), ("call_site", ct.c_ulonglong),("ptr", ct.c_ulonglong),("bytes_req", ct.c_ulonglong),("bytes_alloc", ct.c_ulonglong),("gfp_flags", ct.c_ulonglong), ("command", ct.c_char * 30)]


# Show message when ePBF stats
print("Kernel cache allocation started ... Hit Ctrl-C to end!")

print("%-18s %-20s %-24s %-15s %-12s %-28s %-30s" % ("TIME(S)", "Call_site", "ptr", "bytes_req", "bytes_alloc", "gfp_flags", "Command"))


def display_kernel_alloc_free(cpu, data, size):
    evenement = ct.cast(data, ct.POINTER(Data)).contents
    print("%-18.0f %-20.0f 0x%-22.0f %-15.0f %-12.0f 0x%-28d %-30s" % (evenement.timestamp, evenement.call_site, evenement.ptr, evenement.bytes_req, evenement.bytes_alloc, evenement.gfp_flags, evenement.command))
