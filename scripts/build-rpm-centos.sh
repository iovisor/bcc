...
pushd $TMP
spectool -g -C $TMP/SOURCES SPECS/bcc.spec
rpmbuild --define "_topdir `pwd`" -ba SPECS/bcc.spec --with local_clang_static --with cmake3
popd

mkdir /rpms
cp $TMP/RPMS/*/*.rpm /rpms
cp $TMP/SRPMS/*.rpm /rpms
